...
| 2026-09-25 | Release v1.0.0 archived | – | – | – | Zenodo concept DOI 10.5281/zenodo.22967521, version DOI 10.5281/zenodo.22967522 (a second record of the same release, 10.5281/zenodo.22967647, was produced by the duplicated webhook delivery and is to be removed). DOIs entered in CITATION.cff, the JSON-LD, the About page, the README and the essay (v1.3, manuscript rebuilt). |
| 2026-09-25 | Paths revised, site icon | – | – | – | The editor accepted the seven paths; draft marks removed. Titles of 47-293 and 48-018 corrected to Weingarten (`article_overrides.json`, vols. 47–48 rebuilt, text unchanged). Site icon: the college front of 1871 with the cross and the river (`assets/icon.svg`, also the favicon). |
| 2026-09-25 | Reading paths | – | – | – | Seven draft paths in `data/paths.json` (farms and slaves, the American university, Woodstock itself, the jubilee effect with discourse measures, Indian missions, science, chaplains), 49 stations, all article ids verified by the check script; `#/paths`. Notes marked draft until the editor revises them. |
| 2026-09-25 | Register, tables, checks, the run to 1969 | 2,671 | unchanged | unchanged | Register from the printed general index of 1960 (`tools/build_general_index.py` → `data/general_index.json`: 6,578 headings, 19,389 references, headings and references only, the compiler's phrases discarded because the index is in copyright; `#/register`, with `#/p/VOL/PAGE` resolving a page to its article). Its "(auth.)" marks fill 202 authors of unsigned pieces (`authorFrom: "general index"`); all 59 volumes rebuilt, pages and text unchanged. The fold-out statistics extracted as OCR text and best-effort CSV (`tools/extract_tables.py` → `data/tables/`, `#/tables`: 100 tables, 49 fold-outs and 51 on numbered pages; a text page counts only when the heading opens the leaf). Consistency checks in `tools/check_edition.py` (manifest, page order, plates, essay citations, register, no third-party loads); CLAUDE.md requires them before every commit. The Volumes page lists vols. 60–98 again, catalogue-only, with their rights tier. Essay v1.2 adds the register and the tables. |
| 2026-09-25 | Introduction, plates, legal notice | – | – | – | Introductory essay (`data/introduction.json`, rendered at `#/introduction`, manuscript in `docs/` built by `tools/build_introduction_docx.js`, APA 7; single author, with the use of the AI system disclosed in the author note and a closing section, citing Floridi's *distant writing*); fourteen plates cut from the Golden Jubilee number (vol. 49 no. 1), Dooley's *Woodstock and Its Makers* (vol. 56 no. 1) and the Hopkins atlas of 1877 (Library of Congress), registered in `data/plates.json`, on the home page and at `#/plates`; legal notice and privacy at `#/imprint` (states the one localStorage key and Netlify's injected RUM script); Open Graph and JSON-LD metadata; `CITATION.cff` and `.zenodo.json` for the release. README's article count corrected (2,671). |
