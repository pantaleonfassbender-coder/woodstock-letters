...
| 2026-09-25 | Introduction, plates, legal notice | – | – | – | Introductory essay (`data/introduction.json`, rendered at `#/introduction`, manuscript in `docs/` built by `tools/build_introduction_docx.js`, APA 7, two authors); fourteen plates cut from the Golden Jubilee number (vol. 49 no. 1), Dooley's *Woodstock and Its Makers* (vol. 56 no. 1) and the Hopkins atlas of 1877 (Library of Congress), registered in `data/plates.json`, on the home page and at `#/plates`; legal notice and privacy at `#/imprint` (states the one localStorage key and Netlify's injected RUM script); Open Graph and JSON-LD metadata; `CITATION.cff` and `.zenodo.json` for the release. README's article count corrected (2,671). |
